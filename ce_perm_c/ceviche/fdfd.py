...
from .primitives import sp_solve, sp_mult, spsp_mult, sp_perm_c, sp_solve_with_perm_c
...
class my_fdfd_ez(fdfd):
    ...
    def _solve_fn(self, eps_vec, entries_a, indices_a, Jz_vec):
        ...
        if self.perm_c is None:
            self.perm_c = sp_perm_c(entries_a, indices_a, b_vec)
            # primitives.perm_c = self.perm_c

        Ez_vec = sp_solve_with_perm_c(entries_a, indices_a, b_vec, self.perm_c)
        Hx_vec, Hy_vec = self._Ez_to_Hx_Hy(Ez_vec)
        return Hx_vec, Hy_vec, Ez_vec
