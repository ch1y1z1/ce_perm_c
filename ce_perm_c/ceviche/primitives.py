...
from ce_perm_c import mini_sci
...
def sp_perm_c(entries, indices, b):
    """Compute the permutation vector for a sparse matrix A"""
    N = b.size
    A = make_sparse(entries, indices, shape=(N, N))
    return mini_sci.linsolve.sp_perm_c(A, b)
...
@ag.primitive
def sp_solve_with_perm_c(entries, indices, b, perm_c):
    """Solve a sparse matrix (A) with source (b)
    Args:
      entries: numpy array with shape (num_non_zeros,) giving values for non-zero
        matrix entries.
    ...
    A = make_sparse(entries, indices, shape=(N, N))
    # calls a customizable solving function from ceviche.solvers, could add options to sp_solve() here eventually
    return solve_linear(A, b, perm_c=perm_c)


def grad_sp_solve_entries_reverse(x, entries, indices, b, perm_c):
    # x^T @ dA/de^T @ A_inv^T @ -v => do the solve on the RHS, then take outer product with x using indices of A
    indices_T = transpose_indices(indices)
    i, j = indices
...
def grad_sp_solve_b_reverse(ans, entries, indices, b, perm_c):
    # dx/de^T @ A_inv^T @ v => do the solve on the RHS and you're done.
    indices_T = transpose_indices(indices)
...
ag.extend.defvjp(
    sp_solve_with_perm_c,
    grad_sp_solve_entries_reverse,
    None,
    grad_sp_solve_b_reverse,
    None,
)
