...
def real(val, outline=None, ax=None, cbar=False, cmap='RdBu', outline_alpha=0.5, alpha=1.0, norm=None):
    ...
    vmax = np.abs(val).max()
    h = ax.imshow(np.real(val.T), cmap=cmap, origin='lower', vmin=-vmax, vmax=vmax, alpha=alpha, norm=norm)
